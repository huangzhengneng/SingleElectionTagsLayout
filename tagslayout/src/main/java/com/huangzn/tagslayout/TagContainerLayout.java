...
import static com.huangzn.tagslayout.Utils.dp2px;
import static com.huangzn.tagslayout.Utils.sp2px;
...
public class TagContainerLayout extends ViewGroup {
    ...
    /**
     * The ripple effect True is enable or unEnable, default is false.
     */
    private boolean mRippleEnable = false;
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mRippleEnable = attributes.getBoolean(R.styleable.AndroidTagView_tag_ripple_enable, mRippleEnable);
...
//        int widthSpecMode = MeasureSpec.getMode(widthMeasureSpec);
    ...
    private void initTagView(TagView tagView, int position) {
        ...
        tagView.setRippleEnable(mRippleEnable);
     ...
     * @param position
     */
    public void invalidateTagsWhenClicked(int position) {
        if (isNotEmptyContent()) {
            for (int i = 0; i < mChildViews.size(); i++) {
                View view = mChildViews.get(i);
                // 将当前点击itemview设置为selected，其它itemview设置为unSelected
    ...
    public void selectTag(int position) {
        invalidateTagsWhenClicked(position);
    }
     ...
     * @return
     */
    public boolean isRepeatClick(int currentClickPosition) {
        if (!isNotEmptyContent() || currentClickPosition < 0) {
            return false;
        }
        int lastClickPosition = -1;
        for (int i = 0; i < mChildViews.size(); i++) {
            View tagView = mChildViews.get(i);
            if (tagView != null && tagView.isSelected()) {
                lastClickPosition = i;
                break;
            }
        }
        return lastClickPosition == currentClickPosition;
    }
}
